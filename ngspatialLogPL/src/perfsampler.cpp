...
vec rautologistic(const mat& X, const mat& A, const vec& theta, unsigned int seed)
{
    ...
    while (true)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            q = 1 / (1 + exp(Xbeta[i] + eta * sumL_i));
            if (R(t - 1, i) > q)
                L[i] = 1;
            else
                L[i] = 0;
            sumU_i = as_scalar(A.row(i) * U);
            q = 1 / (1 + exp(Xbeta[i] + eta * sumU_i));
            if (R(t - 1, i) > q)
                U[i] = 1;
            else
                U[i] = 0;

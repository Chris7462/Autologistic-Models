...
rautologistic = function(X, A, theta, seed)
{
    ...
    as.vector(perfsampler$rautologistic(X, A, theta, seed))
}
...
autologistic.logPL = function(theta, X, A, Z)
{
    ...
    eta = theta[p]
    Xbeta = X %*% beta
    # mu has been removed by Yi-Chen 11/26/13 (use non-central model)
    #mu = exp(Xbeta) / (1 + exp(Xbeta))
    #logPL = Xbeta + eta * A %*% (Z - mu)

    # add loglihood correction (Jeffery prior)
    logPL = Xbeta + eta * A %*% Z
    auto.cov <- A %*% Z
    XZ <- cbind(X,auto.cov)
    n <- dim(XZ)[1]
    W <- matrix(0, nrow=n, ncol=n)
    diag(W) <- exp(logPL)/((1+exp(logPL))^2)
    Vrs <- -t(XZ) %*% W %*% XZ
    Irs <- -Vrs
    logPL = t(Z) %*% logPL - sum(log(1 + exp(logPL))) + .5*log(det(Irs))
    -logPL
}
...
autologistic.fit = function(X, A, Z, hessian)
{
    ...
    opt = try(optim(c(start, 1), autologistic.logPL, NULL, X = X, A = A, Z = Z,
...
autologistic.grad = function(params, X, A, Z)
{
    ...
    Xbeta <- X %*% beta
    auto.cov <- A %*% Z
    logPL <- Xbeta + eta * auto.cov

    p = as.vector(exp(logPL))
    p = p / (1 + p)
    ...
    # Third derivative
    B <- array(0, dim=c(len,n,n))
    W <- array(0, dim=c(len,len,len)) 
    for ( r in 1:len ){
        diag(B[r,,]) <- p*(1-p)*(2*p-1)*XZ[,r]
        W[r,,] <- t(XZ) %*% B[r,,] %*% XZ
    }
    tmp <- rep(0,length=len)

    for ( r in 1:len ){
      tmp[r] <- -.5 * sum(diag(solve(Irs)%*%W[r,,]))
    }
...
autologistic.sandwich = function(X, A, theta, type, bootit, parallel, nodes)
{
    ...
    else
    {
        ...
        clusterEvalQ(cl, library(ngspatialLogPL))
